...
# 计算每个音频的 Max 概率 (Max Pooling)
y_prob_audio = np.array([np.max(probs) for probs in all_window_probs_per_audio])
...
for thresh in thresholds:
    ...
    # zero_division=0 防止在某些极端阈值下预测全为 0 导致报错
...
# 打印格式化结果
print("="*55)
print(f"--- 窗口级 Top-{K} SVM 最终测试结果 (阈值={best_threshold:.2f}) ---")
print("="*55)
...
joblib.dump(svm_win, f'models/svm_window_topk{K}_rand_neg.pkl')
joblib.dump(scaler_win, f'models/scaler_window_topk{K}_rand_neg.pkl')

# 保存最佳阈值配置，方便未来推理时直接加载
config = {'best_threshold': float(best_threshold)}
joblib.dump(config, f'models/config_window_topk{K}_rand_neg.pkl')
