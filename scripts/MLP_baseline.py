...
class GlobalMLP(nn.Module):
    """处理全局 1D 特征向量的多层感知机 (MLP) Baseline。"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a CUDA PyTorch MLP model on global MFCC features as a baseline."
    )
...
def train_fold(
    ...
    checkpoint = {
        "model_type": "global_mlp_baseline",
...
def main() -> None:
    ...
    results = {
        ...
        "fold_best_validation_losses": [checkpoint["best_validation_loss"] for checkpoint in checkpoints],
